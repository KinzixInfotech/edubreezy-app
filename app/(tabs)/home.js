...
import { HOME_CACHE_CONFIG } from '../../features/home-modules/cacheConfig';
import { mapParentChildren } from '../../features/home-modules/parentUtils';
import StatusModals from '../../features/home-modules/StatusModals';
import { useHomeProfile } from '../../features/home-modules/useHomeProfile';
import { useHomeRefresh } from '../../features/home-modules/useHomeRefresh';
import HomeHeader from '../../features/home-modules/HomeHeader';
import ParentHome from '../../features/home-modules/roles/ParentHome';
...
export default function HomeScreen() {
    ...
    const userRole = user_acc?.role?.name;
    const parentId = user_acc?.parentData?.id;
    ...
    const { data: notificationData, refetch: refetchNotifications } = useQuery({
        queryKey: homeQueryKeys.notificationsSummary(userId, schoolId),
        queryFn: async () => {
    ...
    const { data: dashboardData } = useQuery({
        queryKey: homeQueryKeys.teacherDashboard(schoolId, userId),
    ...
    const { data: transportStaffData, isLoading: isTransportStaffLoading } = useQuery({
        queryKey: homeQueryKeys.transportStaff(schoolId, userId),
        queryFn: async () => {
    ...
    const { data: transportTripsData, isLoading: isTransportTripsLoading } = useQuery({
        queryKey: homeQueryKeys.transportTrips(schoolId, transportStaffId),
        queryFn: async () => {
    ...
    const { data: upcomingEventsData, isLoading: eventsLoading } = useQuery({
        queryKey: homeQueryKeys.upcomingEvents(schoolId),
        queryFn: async () => {
    ...
    if (!user_acc?.id || !schoolId || !user_acc?.role?.name) {
        return (
            <View style={styles.loaderContainer}>
        ...
        switch (role) {
            ...
            case 'parent':
                return <ParentHome
                    ...
                    PARENT_QUICK_STAT_VALUE_FONT_SIZE={PARENT_QUICK_STAT_VALUE_FONT_SIZE}
                    PARENT_QUICK_STAT_LABEL_FONT_SIZE={PARENT_QUICK_STAT_LABEL_FONT_SIZE}
                />
    ...
    const StudentView = ({ refreshing, onRefresh, banner, paddingTop, refreshOffset, navigateOnce }) => {
        ...
        const { data: dashboardData, isLoading: isDashboardLoading } = useQuery({
            queryKey: homeQueryKeys.studentDashboard(schoolId, userId),
            queryFn: async () => {
    ...
    // === DIRECTOR VIEW ===
    const DirectorView = ({ refreshing, onRefresh, schoolId, userId, paddingTop, refreshOffset, navigateOnce }) => {
        // Fetch academic years first (like admin web dashboard)
        const { data: academicYears, isLoading: isLoadingYears } = useQuery({
            queryKey: homeQueryKeys.academicYears(schoolId),
            queryFn: async () => {
        ...
        const { data: apiStats, isLoading, error } = useQuery({
            queryKey: homeQueryKeys.dashboardOverview(schoolId, academicYearId),
            queryFn: async () => {
        ...
        const { data: feeStats } = useQuery({
            queryKey: homeQueryKeys.feeDashboard(schoolId, academicYearId),
            queryFn: async () => {
        ...
        const { data: recentNotices } = useQuery({
            queryKey: homeQueryKeys.directorNotices(schoolId, userId),
            queryFn: async () => {
    ...
    // === PRINCIPAL VIEW ===
    const PrincipalView = ({ refreshing, onRefresh, paddingTop, schoolId, userId, refreshOffset, navigateOnce }) => {
        // Fetch academic years first (like admin web dashboard)
        const { data: academicYears, isLoading: isLoadingYears } = useQuery({
            queryKey: homeQueryKeys.academicYears(schoolId),
            queryFn: async () => {
        ...
        const { data: recentNotices } = useQuery({
            queryKey: homeQueryKeys.principalNotices(schoolId, userId),
            queryFn: async () => {
                if (!schoolId || !userId) return { notices: [] };
                const res = await api.get(`/notices/${schoolId}?userId=${userId}&limit=4&page=1`);
                return res.data;
    ...
    const AccountantView = ({ refreshing, onRefresh, schoolId, userId, paddingTop, refreshOffset, banner, navigateOnce }) => {
        const { data: dashboardData, isLoading, refetch } = useQuery({
            queryKey: homeQueryKeys.accountantDashboard(schoolId, userId),
            queryFn: async () => {
                if (!schoolId || !userId) return null;
    ...
    return (
        <View style={styles.container} edges={['top']}>
            ...
            <HomeHeader
                ...
                navigateOnce={navigateOnce}
                onRefresh={onRefresh}
                replaceDynamic={replaceDynamic}
                getGreeting={getGreeting}
                getInitials={getInitials}
                headerAnimatedStyle={headerAnimatedStyle}
                headerContentOpacity={headerContentOpacity}
                headerNameStyle={headerNameStyle}
                headerIconsTranslate={headerIconsTranslate}
...
const TeacherView = memo(({ schoolId, userId, teacher, refreshing, onRefresh, upcomingEvents, todaysEvents, banner, onScroll, paddingTop, refreshOffset, navigateOnce, onStatusPress, onMyStatusPress }) => {
    ...
    const [showLocationDisclosure, setShowLocationDisclosure] = useState(false); // Disclosure modal state
    ...
    const startOnDemandTrip = async (tripType) => {
        // Show disclosure FIRST (Google Play requires disclosure before location permission)
        pendingTripActionRef.current = async () => {
            Alert.alert(
                `Start ${tripType} Trip?`,
    ...
    const { data: recentNotices } = useQuery({
        queryKey: homeQueryKeys.driverNotices(schoolId, userId),
        queryFn: async () => {
            if (!schoolId || !userId) return { notices: [] };
            const res = await api.get(`/notices/${schoolId}?userId=${userId}&limit=4&page=1`);
            return res.data;
        ...
        <>
            ...
            <LocationDisclosureModal
                ...
                onAccept={() => {
                    ...
                    // Execute the pending trip action (start scheduled or on-demand trip)
                    if (pendingTripActionRef.current) {
                        pendingTripActionRef.current();
                        pendingTripActionRef.current = null;
                    }
                }}
...
const ConductorView = ({ refreshing, onRefresh, onScroll, paddingTop, refreshOffset, schoolId, userId, prefetchedStaffData, prefetchedTripsData, navigateOnce, upcomingEvents, user_acc, banner }) => {
    ...
    const { data: recentNotices } = useQuery({
        queryKey: homeQueryKeys.conductorNotices(schoolId, userId),
        queryFn: async () => {
            if (!schoolId || !userId) return { notices: [] };
            const res = await api.get(`/notices/${schoolId}?userId=${userId}&limit=4&page=1`);
            return res.data;

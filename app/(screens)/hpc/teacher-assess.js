        ...
        <View style={styles.gradeSelector}>
            ...
            <HapticTouchable onPress={() => setIsOpen(!isOpen)}>
                <View style={[
                    ...
                    <Text style={[styles.gradeButtonText, { color: selectedOption?.color || '#666' }]}>
            ...
            {isOpen && (
                <Animated.View entering={FadeInDown.duration(200)} style={styles.gradeDropdown}>
                    {GRADE_OPTIONS.map((option) => (
                        <HapticTouchable key={option.value} onPress={() => { onChange(option.value); setIsOpen(false); }}>
                            <View style={[styles.gradeOption, value === option.value && { backgroundColor: option.color + '20' }]}>
                                <View style={[styles.gradeDot, { backgroundColor: option.color }]} />
                                <Text style={[styles.gradeOptionText, { color: option.color }]}>{option.label}</Text>
                                {value === option.value && <CheckCircle size={16} color={option.color} />}
                            </View>
                        </HapticTouchable>
                    ))}
                </Animated.View>
...
export default function TeacherSELAssessScreen() {
    ...
    const { data: existingAssessments, isLoading: assessmentsLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await api.get(`/schools/${schoolId}/hpc/sel/assess?${queryParams}`);
    ...
    useEffect(() => {
        if (existingAssessments && existingAssessments.length > 0) {
            const initial = {};
            existingAssessments.forEach((a) => { initial[a.parameterId] = a.grade; });
    ...
    const submitMutation = useMutation({
        mutationFn: async (data) => {
            const assessmentArray = Object.entries(data).map(([parameterId, grade]) => ({
                studentId, parameterId, academicYearId: academicYear?.id, termNumber, grade,
            }));
        ...
        onError: (error) => { Alert.alert('Error', error.message || 'Failed to save assessments'); },
    });
    ...
    const handleGradeChange = (parameterId, grade) => {
        setAssessments((prev) => ({ ...prev, [parameterId]: grade }));
    };

    const handleSubmit = () => {
        if (!studentId) {
            Alert.alert('Error', 'Student ID is missing. Please go back and select the student again.', [{ text: 'OK', onPress: () => router.back() }]);
            return;
        }
        const filledCount = Object.keys(assessments).length;
        const totalParams = selParams?.length || 0;
        if (filledCount < totalParams) {
            Alert.alert('Incomplete', `You've assessed ${filledCount} of ${totalParams} parameters. Save anyway?`, [
        ...
        <SafeAreaView style={styles.container} edges={['left', 'right']}>
            ...
            >
                ...
                <View style={styles.headerRow}>
                    ...
                    <HapticTouchable onPress={handleSubmit} disabled={submitMutation.isPending || assessedCount === 0}>
                        <View style={[styles.saveButton, (submitMutation.isPending || assessedCount === 0) && { opacity: 0.5 }]}>
                            {submitMutation.isPending ? <ActivityIndicator size="small" color="#EC4899" /> : <Save size={20} color="#EC4899" />}
                        </View>
                    </HapticTouchable>
                </View>
                ...
                <View style={styles.progressCard}>
                    <View style={styles.progressInfo}>
                        ...
                        <Text style={styles.progressText}>{assessedCount} of {totalParams} assessed</Text>
                    </View>
                    <View style={styles.progressBar}>
                        <View style={[styles.progressFill, { width: `${totalParams > 0 ? (assessedCount / totalParams) * 100 : 0}%` }]} />
                    </View>
                </View>
            </LinearGradient>
            ...
            >
                {Object.entries(groupedParams).map(([category, catParams], catIdx) => (
                    <Animated.View key={category || `cat-${catIdx}`} entering={FadeInDown.delay(catIdx * 100)} style={styles.categoryCard}>
                        <View style={styles.categoryHeader}>
                            <Text style={styles.categoryTitle}>{category}</Text>
                            <Text style={styles.categoryCount}>{catParams.length} parameters</Text>
                        </View>
                        {catParams.map((param, idx) => (
                            <Animated.View key={param?.id || `param-${catIdx}-${idx}`} entering={FadeInRight.delay(catIdx * 100 + idx * 50)}>
                                <GradeSelector
                ...
                <HapticTouchable onPress={handleSubmit} disabled={submitMutation.isPending || assessedCount === 0} style={{ marginTop: 16 }}>
                    <LinearGradient
                        colors={assessedCount > 0 ? ['#EC4899', '#DB2777'] : ['#D1D5DB', '#9CA3AF']}
                        style={styles.submitButton}
                    >
                        {submitMutation.isPending ? <ActivityIndicator size="small" color="#fff" /> : (
                            <><CheckCircle size={20} color="#fff" /><Text style={styles.submitText}>Save Assessments</Text></>
                        )}
                    </LinearGradient>
                </HapticTouchable>
            </ScrollView>
            ...
            <Animated.View entering={FadeInDown.delay(300).duration(400)} style={[styles.stickyBottom, { paddingBottom: insets.bottom + 8 }]}>
                <HapticTouchable onPress={() => router.push({ pathname: '/hpc/teacher-narrative', params: { childData: JSON.stringify(childData), termNumber } })}>
                    <LinearGradient colors={['#8B5CF6', '#7C3AED']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.narrativeButton}>
                        <View style={styles.narrativeIconBox}><MessageSquare size={24} color="#fff" /></View>
...
const styles = StyleSheet.create({
    ...
    backButtonCenter: { marginTop: 20, paddingHorizontal: 24, paddingVertical: 12, backgroundColor: '#EC4899', borderRadius: 12 },
    ...
    progressInfo: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 10 },
    ...
    categoryCard: { backgroundColor: '#fff', borderRadius: 16, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 },
    categoryHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: '#f0f0f0' },
    categoryTitle: { fontSize: 16, fontWeight: '700', color: '#EC4899' },
    categoryCount: { fontSize: 12, color: '#999' },
    gradeSelector: { marginBottom: 14 },
    paramName: { fontSize: 14, fontWeight: '600', color: '#333', marginBottom: 8 },
    gradeButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 14, paddingVertical: 12, borderRadius: 10 },
    gradeButtonText: { fontSize: 13, fontWeight: '600' },
    gradeDropdown: { marginTop: 8, backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#f0f0f0', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 12, elevation: 4, overflow: 'hidden' },
    gradeOption: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f5f5f5' },
    gradeDot: { width: 10, height: 10, borderRadius: 5, marginRight: 10 },
    gradeOptionText: { flex: 1, fontSize: 13, fontWeight: '600' },
    ...
    termSwitcherMini: { flexDirection: 'row', marginTop: 6, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 20, padding: 2, gap: 2 },
    termPill: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 16 },
    termPillActive: { backgroundColor: '#fff' },
    termPillText: { fontSize: 10, fontWeight: '700', color: 'rgba(255,255,255,0.8)' },
    termPillTextActive: { color: '#EC4899' },
    stickyBottom: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: 16, backgroundColor: '#f8f9fa' },
    narrativeButton: { flexDirection: 'row', alignItems: 'center', padding: 16, borderRadius: 16 },
    narrativeIconBox: { width: 48, height: 48, borderRadius: 24, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },
    narrativeTextBox: { flex: 1, marginLeft: 14 },
    narrativeTitle: { fontSize: 16, fontWeight: '700', color: '#fff' },
    narrativeSubtitle: { fontSize: 12, color: 'rgba(255,255,255,0.85)', marginTop: 2 },
});
                    ...
                    <Animated.View entering={FadeInDown.delay(100)} style={styles.section}>
                        ...
                        <View style={styles.card}>
                            <View style={styles.cardHeader}>
                                ...
                                <Text style={styles.dateText}>{new Date(studentReflection.submittedAt).toLocaleDateString()}</Text>
                            </View>
                            {studentReflection.proudMoments && <View style={styles.feedbackItem}><Text style={styles.label}>Proud Moments</Text><Text style={styles.value}>{studentReflection.proudMoments}</Text></View>}
                            {studentReflection.challenges && <View style={styles.feedbackItem}><Text style={styles.label}>Challenges Faced</Text><Text style={styles.value}>{studentReflection.challenges}</Text></View>}
                            {studentReflection.goals && <View style={styles.feedbackItem}><Text style={styles.label}>My Goals</Text><Text style={styles.value}>{studentReflection.goals}</Text></View>}
                            {studentReflection.helpNeeded && <View style={styles.feedbackItem}><Text style={styles.label}>Support Needed</Text><Text style={styles.value}>{studentReflection.helpNeeded}</Text></View>}
                        </View>
                    </Animated.View>
                )}
                            ...
                            <View key={fb.id || index} style={styles.card}>
                                <View style={styles.cardHeader}>
                                    <View style={styles.userInfo}>
                                        <View style={styles.avatar}><User size={16} color="#666" /></View>
                                        <Text style={styles.userName}>{fb.parent?.user?.name || 'Parent'}</Text>
                                    </View>
                                    <Text style={styles.dateText}>{new Date(fb.submittedAt).toLocaleDateString()}</Text>
                                </View>
                                {fb.observations && <View style={styles.feedbackItem}><Text style={styles.label}>Observations</Text><Text style={styles.value}>{fb.observations}</Text></View>}
                                {fb.suggestions && <View style={styles.feedbackItem}><Text style={styles.label}>Suggestions</Text><Text style={styles.value}>{fb.suggestions}</Text></View>}
                                <View style={styles.row}>
                                    {fb.childInterest && <View style={styles.badgeItem}><Text style={styles.badgeLabel}>Interest</Text><Text style={styles.badgeValue}>{fb.childInterest}</Text></View>}
                                    {fb.homeParticipation && <View style={styles.badgeItem}><Text style={styles.badgeLabel}>Participation</Text><Text style={styles.badgeValue}>{fb.homeParticipation}</Text></View>}
                                </View>
                            </View>
                        ))}
                    </Animated.View>
...
const styles = StyleSheet.create({
    ...
    header: { paddingHorizontal: 16, paddingBottom: 20, borderBottomLeftRadius: 24, borderBottomRightRadius: 24, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 4 },
    ...
    emptyState: { alignItems: 'center', justifyContent: 'center', paddingVertical: 60, opacity: 0.7 },
    ...
    card: { backgroundColor: '#fff', borderRadius: 16, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2, borderWidth: 1, borderColor: '#f1f5f9' },
    ...
    label: { fontSize: 11, color: '#94a3b8', fontWeight: '600', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 },
    value: { fontSize: 14, color: '#334155', lineHeight: 22 },
    row: { flexDirection: 'row', gap: 12, marginTop: 4 },
    badgeItem: { backgroundColor: '#f8fafc', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, borderWidth: 1, borderColor: '#e2e8f0', flex: 1 },
    badgeLabel: { fontSize: 10, color: '#64748b', marginBottom: 2 },
    badgeValue: { fontSize: 13, fontWeight: '600', color: '#0f172a' },
});
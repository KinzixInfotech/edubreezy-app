const DriverView = ({ refreshing, onRefresh, onScroll, paddingTop, refreshOffset, schoolId, userId, prefetchedStaffData, prefetchedTripsData, navigateOnce, upcomingEvents, user_acc }) => {
    ...
    // Helper: get trip date label
    const getTripDateLabel = (trip) => {
        const tripDate = new Date(trip.scheduledDate || trip.date || trip.createdAt).toISOString().split('T')[0];
        if (tripDate === today) return 'Today';
        if (tripDate === tomorrowStr) return 'Tomorrow';
        return new Date(tripDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' });
    };
    ...
    // Tomorrow's trips (separate section)
    const tomorrowTrips = trips.filter(t => {
        const tripDate = new Date(t.scheduledDate || t.date || t.createdAt).toISOString().split('T')[0];
        return tripDate === tomorrowStr && t.status === 'SCHEDULED';
    });
                        ...
                        <View style={{ backgroundColor: 'rgba(0,0,0,0.1)', borderRadius: 16, padding: 16 }}>
                            ...
                            {todaysScheduledTrips.length > 0 && (
                                <View style={{ marginTop: 16, paddingTop: 16, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.15)' }}>
                                    <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)', marginBottom: 10 }}>Today's Trips</Text>
                                    {todaysScheduledTrips.map((trip, idx) => {
                                        ...
                                        return (
                                            <View key={trip.id} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: idx < todaysScheduledTrips.length - 1 ? 8 : 0 }}>
                                                ...
                                                <Text style={{ fontSize: 14, color: '#fff', marginLeft: 8, flex: 1 }}>
                                                    {trip.tripType}{tripTime ? ` • ${tripTime}` : ''} • {trip.route?.name}
                                                </Text>

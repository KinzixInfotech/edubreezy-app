...
export async function saveProfile(schoolCode, profileData, sessionData = null) {
    try {
        ...
        const minimalUserData = {
            ...
            loginIdentifier: profileData.loginIdentifier,
            loginRole: profileData.loginRole,
        ...
        const profile = {
            ...
            email: profileData.loginIdentifier || profileData.email || profileData.parentData?.email || '',
            profilePicture: profileData.profilePicture,
            schoolId: profileData.schoolId,
        ...
        console.log('✅ Saved profile metadata to SecureStore');
...
// Clear all saved profiles on this device
...
// Update profile picture for a specific user across all school entries
export async function updateProfilePicture(userId, photoUrl) {
    try {
        ...
        // Iterate through all schools and update the user's picture everywhere they appear
        Object.keys(allProfiles).forEach(schoolCode => {
            allProfiles[schoolCode] = allProfiles[schoolCode].map(profile => {
        ...
        if (updated) {
            console.log(`📸 Updated profile picture for user ${userId} in SecureStore`);
            await SecureStore.setItemAsync(PROFILES_KEY, JSON.stringify(allProfiles));
        }
    } catch (error) {
...
// Clear stale/invalid session tokens from a profile (when refresh fails)
export async function clearProfileSession(schoolCode, userId) {
    try {
        ...
        if (profileIndex >= 0) {
            console.log('🗑️ Clearing stale session tokens for user:', userId);

            // Remove the session tokens entirely
            delete allProfiles[schoolCode][profileIndex].sessionTokens;

            await SecureStore.setItemAsync(PROFILES_KEY, JSON.stringify(allProfiles));
            console.log('✅ Stale tokens cleared from profile');
        }
    } catch (error) {

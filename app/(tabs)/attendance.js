...
export default function SelfAttendance() {
    ...
    const { data: regData } = useQuery({
        queryKey: ['regularization-requests', userId, schoolId],
        queryFn: async () => {
            const res = await api.get(`/schools/${schoolId}/attendance/admin/regularization?userId=${userId}&status=PENDING,APPROVED,REJECTED`);
            return res.data;
        },
        enabled: !!userId && !!schoolId,
    });
    ...
    return (
        ...
        >
            ...
            {/* Add styles for infoGrid if missing */}
            <View style={{ height: 40 }} />
            ...
            {regData?.requests && regData.requests.length > 0 && (
                <Animated.View entering={FadeInDown.delay(650)} style={styles.requestSection}>

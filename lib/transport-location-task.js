...
// CRITICAL: Set notification handler at module level so it's registered
// before any notification is scheduled. Without this, foreground notifications
// are silently swallowed by expo-notifications.
...
// Create the trip-tracking notification channel on Android at import time.
// Android 8+ requires a channel before any notification can be shown.
if (Platform.OS === 'android') {
    Notifications.setNotificationChannelAsync('trip-tracking', {
        name: 'Trip Tracking',
        description: 'Persistent notification while a bus trip is active',
        importance: Notifications.AndroidImportance.HIGH,
        lockscreenVisibility: Notifications.AndroidNotificationVisibility.PUBLIC,
        sound: null,
    }).then(() => console.log('✅ trip-tracking channel created'))
        .catch(e => console.warn('⚠️ Failed to create trip-tracking channel:', e));
}
...
async function showTripNotification(routeName) {
    // Android 13+ (API 33) requires POST_NOTIFICATIONS permission for local notifications
    ...
    if (existingStatus !== 'granted') {
        ...
        if (status !== 'granted') {
            console.warn('⚠️ Notification permission not granted — tracking notification won\'t show');
            return;
        }
    }
    ...
    try {
        await Notifications.scheduleNotificationAsync({
            ...
            content: {
                ...
                ...(Platform.OS === 'android' ? { channelId: 'trip-tracking' } : {}),
            },
            trigger: null, // Fire immediately
        });
        console.log('✅ Trip tracking notification scheduled successfully');
    } catch (err) {
        console.error('❌ Failed to schedule trip notification:', err);
    }
}

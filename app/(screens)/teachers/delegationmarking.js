// app/(screens)/teacher/mark-attendance-delegation.jsx
// This is identical to your bulk marking page but with delegation support
...
export default function DelegationAttendanceMarking() {
    ...
    const { data: studentsData, isLoading: studentsLoading } = useQuery({
        queryKey: ['delegation-students', schoolId, classId, sectionId, selectedDate],
        queryFn: async () => {
            ...
            try {
                const params = new URLSearchParams({
                    ...
                    date: selectedDate,
                    ...(sectionId && sectionId !== 'all' && { sectionId: sectionId.toString() })
                });
    ...
    // Submit bulk attendance with delegation support
    const submitMutation = useMutation({
        mutationFn: async (data) => {
            const res = await api.post(`/schools/${schoolId}/attendance/bulk`, {
                ...data,
                delegationId: delegationId || null // Include delegation ID
            });
            return res.data;
        },
        onSuccess: () => {
            queryClient.invalidateQueries(['delegation-students']);
            setHasChanges(false);
        ...
        onError: (error) => {
            ...
            if (errorData) {
                ...
                Alert.alert('Error', 'Failed to mark attendance. Please try again.', [{ text: 'OK' }]);
            }
        }
    });
    ...
    const onRefresh = useCallback(async () => {
        setRefreshing(true);
        await queryClient.invalidateQueries(['delegation-students']);
        setRefreshing(false);
    }, [queryClient]);
        ...
        <View style={styles.container}>
            ...
            >
                <View style={styles.delegationBadge}>
                        ...
                        <Text style={styles.delegationBadgeText}>Substitute</Text>
                    </View>
                {/* Date & Info Card */}
                <Animated.View entering={FadeInDown.delay(200).duration(400)} style={styles.classInfoCard}>
                    ...
                    {isDelegation === 'true' && (
                        <View style={styles.delegationAlert}>
                            <Shield size={20} color="#0469ff" />
                            <View style={styles.delegationAlertContent}>
                                <Text style={styles.delegationAlertTitle}>Substitute Assignment</Text>
                                <Text style={styles.delegationAlertText}>
                                    You are marking attendance as a substitute teacher
                                </Text>
                            </View>
                        </View>
                    )}
...
const styles = StyleSheet.create({
    // ... (use the same styles from your original bulk marking component)
    ...
    delegationAlertContent: {
        flex: 1,
    },
    delegationAlertTitle: {
        fontSize: 14,
        fontWeight: '600',
        color: '#1E40AF',
        marginBottom: 2,

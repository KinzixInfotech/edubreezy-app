...
export async function getSchoolLocation(schoolId, apiBaseUrl = null, options = {}) {
    ...
    try {
        ...
        if (attendanceResponse.ok) {
            ...
            if (config && config.enableGeoFencing && config.schoolLatitude && config.schoolLongitude) {
                const location = {
                    ...
                    radiusMeters: config.allowedRadiusMeters ?? config.allowedRadius ?? null,
                    attendanceRadius: config.allowedRadiusMeters ?? config.allowedRadius ?? null,
        ...
        if (response.ok) {
            ...
            const location = {
                ...
                radiusMeters: settings.geofenceRadius ?? null,
                attendanceRadius: settings.attendanceRadius ?? null,
...
// ============================================
// APPROACHING STOP DETECTION (for notifications)
// ============================================

const APPROACHING_ALERT_M = 500;    // Send "approaching" notification
const APPROACHING_IMMINENT_M = 200; // Send "arriving now" notification

/**
 * Check which uncompleted stops the bus is approaching
 * Returns stops within alert (500m) and imminent (200m) zones
 * 
 * @param {number} lat - Bus latitude
 * @param {number} lon - Bus longitude
 * @param {Array} stops - All stops with {id, latitude, longitude, name}
 * @param {Array} completedStopIds - Already completed stop IDs
 * @returns {Array<{stop, distance, zone: 'alert'|'imminent'}>}
 */
export function checkApproachingStops(lat, lon, stops, completedStopIds = []) {
    const results = [];
    const completedSet = new Set(completedStopIds);

    for (const stop of stops) {
        if (!stop.latitude || !stop.longitude) continue;
        if (completedSet.has(stop.id)) continue;

        const distance = getDistanceMeters(lat, lon, stop.latitude, stop.longitude);

        if (distance <= APPROACHING_IMMINENT_M) {
            results.push({ stop, distance: Math.round(distance), zone: 'imminent' });
        } else if (distance <= APPROACHING_ALERT_M) {
            results.push({ stop, distance: Math.round(distance), zone: 'alert' });
        }
    }
...
/**
 * Check if a notification should be triggered for a stop
 * Prevents duplicate notifications for the same stop+zone in a single trip
 * 
 * @param {string} stopId
 * @param {string} zone - 'alert' or 'imminent'
 * @param {Object} notifiedStops - Map of { stopId: Set<zone> }
 * @returns {boolean}
 */
export function shouldTriggerNotification(stopId, zone, notifiedStops) {
    if (!notifiedStops || !stopId) return true;

    const notifiedZones = notifiedStops[stopId];
    if (!notifiedZones) return true;

    return !notifiedZones.includes(zone);
}

/**
 * Get the notified stops record from SecureStore for a trip
 * Format: { stopId: ['alert', 'imminent'] }
 */
export async function getNotifiedStopsForTrip(tripId) {
    try {
        const key = `trip_${tripId}_notified_stops`;
        const data = await SecureStore.getItemAsync(key);
        return data ? JSON.parse(data) : {};
    } catch (e) {
        console.error('Error reading notified stops:', e);
        return {};
    }
}

/**
 * Mark a stop+zone as notified in SecureStore
 */
export async function markStopNotified(tripId, stopId, zone) {
    try {
        const key = `trip_${tripId}_notified_stops`;
        const current = await getNotifiedStopsForTrip(tripId);

        if (!current[stopId]) {
            current[stopId] = [];
        }
        if (!current[stopId].includes(zone)) {
            current[stopId].push(zone);
        }

        await SecureStore.setItemAsync(key, JSON.stringify(current));
    } catch (e) {
        console.error('Error marking stop notified:', e);
    }
}

/**
 * Clear notified stops for a trip (call when trip ends)
 */
export async function clearNotifiedStops(tripId) {
    try {
        const key = `trip_${tripId}_notified_stops`;
        await SecureStore.deleteItemAsync(key);
    } catch (e) {
        console.error('Error clearing notified stops:', e);
    }
}

// app/(screens)/principal/leave-management.js
...
export default function PrincipalLeaveManagement() {
    ...
    const { data: permissions, refetch: refetchPermissions } = useQuery({
        queryKey: ['leave-permissions-principal', schoolId],
        queryFn: async () => {
            const res = await api.get(`/schools/${schoolId}/attendance/leave-permissions?viewerRole=PRINCIPAL`);
            return res.data;
        },
        enabled: !!schoolId,
        staleTime: 300 * 1000,
    ...
    const canApprove = permissions?.viewer?.canApprove;
    const isReadOnly = permissions?.viewer?.isReadOnly;
    ...
    const handleApprove = (leave) => {
        if (!canApprove) {
            Alert.alert('Read Only', 'Leave approval has been restricted by the Director.');
            return;
        }
                    ...
                    {activeStatus === 'PENDING' && (
                        canApprove ? (
                            ...
                            <View style={styles.readOnlyBadge}>
                                ...
                                <Text style={styles.readOnlyText}>View Only (Approval restricted)</Text>
                            </View>
                        )
                    )}
            ...
            {isReadOnly && (
                <View style={styles.readOnlyBanner}>
                    ...
                    <Text style={styles.readOnlyBannerText}>
                        Approval restricted by Director. View only mode.
                    </Text>
                </View>
            )}
